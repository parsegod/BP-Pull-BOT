    ...
    for weapon in data["Weapons"]:
        ...
        for bp in weapon["Blueprints"]:
            if bp["Pool"] == pool_number:
                ...
                display_label = f"(Pool {bp['Pool']}) ({weapon['Name']}) {blueprint_name_cleaned} [{bp.get('status', 'RELEASED').replace("'", "")}]"
    ...
    for weapon in data["Weapons"]:
        for bp in weapon["Blueprints"]:
            ...
            if blueprint_status == selected_status:
                ...
                blueprint_name_cleaned = re.sub(r'\*\*(.*?)\*\*', r'\1', bp['Name']).replace('*', '').strip()
      display_label = f"(Pool {bp.get('Pool', 'N/A')}) ({weapon['Name']}) {blueprint_name_cleaned} [{blueprint_status.replace("'", "")}]"
